...
public class WebSocket extends EventDispatcher {
  ...
  public override function WebSocket(
      id:int, url:String, protocols:Array, origin:String,
      proxyHost:String, proxyPort:int,
      cookie:String, headers:String,
      logger:IWebSocketLogger, useFlashSecureSocket:Boolean) {
    this.logger = logger;
    this.id = id;
    this.url = url;
  ...
  public function close(
    ...
    if (code != STATUS_NORMAL_CLOSURE &&
      ...
      logger.error("Fail connection by " + origin + ": code=" + code + " reason=" + reason);
    }
  ...
  private function onSocketIoError(event:IOErrorEvent):void {
	...
	// GP: Ugly way to catch the error 2048, strangely its errorID is 0
	if ((event.errorID == STATUS_SOCKET_ERROR || event.text.indexOf("2048") >= 0) && useFlashSecureSocket == true)
	{
		useFlashSecureSocket = false;
		
		if (secureSocket != null)
		{
			logger.error("Server Certificate Status: " + secureSocket.serverCertificateStatus);
			/*var serverCertificate:flash.security.X509Certificate = secureSocket.serverCertificate;
			if (serverCertificate != null)
			{
				logger.error("issuerUniqueId: " + serverCertificate.issuerUniqueID);
				logger.error("serialNumber: " + serverCertificate.serialNumber);
				logger.error("subjectPublicKey: " + serverCertificate.subjectPublicKey);
				logger.error("subjectPublicKeyAlgorithmOID: " + serverCertificate.subjectPublicKeyAlgorithmOID);
				logger.error("subjectUniqueId: " + serverCertificate.subjectUniqueID);
				logger.error("validNotAfter: " + serverCertificate.validNotAfter.toString());
				logger.error("validNotBefore: " + serverCertificate.validNotBefore.toString());
				logger.error("version: " + serverCertificate.version);
			}*/
		}
	}
  }
          ...
          if (frame.rsv != 0) {
            ...
            switch (frame.opcode) {
              ...
              case OPCODE_PING:
                ...
                break;
              case OPCODE_PONG:
				  var pongEvent:WebSocketEvent  = new WebSocketEvent("pong");
  ...
  public function sendPing(payload:ByteArray):Boolean {
	  var frame:WebSocketFrame = new WebSocketFrame();
  ...
  private function generateKey():String {
	...
	return com.hurlant.util.Base64.encodeByteArray(vals);
  }
